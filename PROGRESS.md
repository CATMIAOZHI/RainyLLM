...
### 2.2 对话管理
- [-] 2.2.1 创建 `engine/ConversationPool.kt`：管理多个客户端会话 <!-- 设计变更：改为 OpenAIServer 中每次请求 createConversation().use{} 用完即毁模式 -->
- [-] 2.2.2 实现会话创建、查找、销毁方法 <!-- 同上，不需要独立会话池 -->
- [-] 2.2.3 实现会话超时自动清理（如 5 分钟无活动） <!-- 同上 -->
- [x] 2.2.4 实现 `ConversationConfig` 构建器（systemInstruction, samplerConfig） <!-- 完成于 OpenAIServer.kt，构建在 handleSyncResponse/handleStreamResponse 中 -->
